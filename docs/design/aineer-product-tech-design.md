...
**AI Agent 模式（碾压竞品的核心差异化）**
6. **自主多步执行** -- 用户下达高级意图（如 "把这个 PR 部署到 staging"），AI 自动规划步骤（git pull → build → test → deploy）、逐步执行、实时汇报进度
7. **工具链编排** -- Agent 可组合调用 shell 命令、MCP 工具、文件操作、git 操作，形成完整工作流，每一步都在 Block 流中可视化
8. **人机协作审批** -- 危险操作（rm -rf、force push、生产部署）需要用户确认；用户可配置审批策略（全部审批 / 仅危险命令 / 全自动）
9. **Plan+Execute 可视化** -- Agent 先展示执行计划（checklist），用户可编辑/调整，确认后自动执行；执行过程中每步状态实时更新（pending → running → done/failed）

**Rich Chat 能力**（不是简陋的文本框，而是完整的聊天体验）
10. **附件** -- 拖拽/粘贴文件到输入框，作为 AI 上下文（代码文件、日志文件、配置文件）
11. **截图/图片** -- Cmd+Shift+S 截屏 或 粘贴剪贴板图片 → 多模态 AI 分析（UI 截图、错误截图、架构图）
12. **@ 引用** -- @file 引用文件、@block 引用历史 Block、@git 引用 diff、@url 引用网页、@memory 引用记忆
13. **/ 命令** -- /fix、/explain、/test、/deploy、/agent 等快捷指令，带自动补全和参数提示
14. **上下文芯片** -- 附件、引用以可视化芯片（Chip）显示在输入框上方，可单独删除
15. **消息内模型切换** -- 对话中可 @model:gpt-4o 随时切换模型，或对同一问题 [Ask Another Model] 获取第二意见

**主动式 AI 行为（不止错误诊断）**
16. **长任务监控** -- 命令运行超过设定阈值（默认 30s）→ AI 气泡提示 "这个命令已运行 2 分钟，要我分析原因吗？"
17. **重复模式识别** -- 检测到用户重复执行相似命令 3 次 → 建议 "要我帮你写成脚本吗？"
18. **危险命令拦截** -- 识别 `rm -rf /`、`DROP TABLE` 等高危命令 → 弹出确认气泡 + AI 解释风险
19. **智能补全增强** -- 基于项目上下文和历史命令的 fish-style ghost text 补全，Tab 接受

**全局搜索（Cmd+P / Cmd+Shift+F）**
20. **Block 搜索** -- 搜索所有历史 Block（命令、AI 对话、工具输出），按时间/类型/关键词过滤
21. **跨 Session 搜索** -- 不仅搜索当前 Tab，还能搜索所有历史 Session 的内容
22. **语义搜索** -- 不仅精确匹配，还支持自然语言搜索（如 "上次修 Dockerfile 那个问题"），由 Memory 系统支撑
23. **Command Palette** -- Cmd+Shift+P 打开命令面板，搜索所有命令、设置、快捷键，与 VS Code 一致的肌肉记忆

**上下文面板（Context Panel）**
24. **AI 上下文可视化** -- Sidebar 可切换到 Context 面板，展示 AI 当前 "看到" 的所有上下文：最近 N 条 Block、引用的文件、记忆片段、系统提示词
25. **上下文预算** -- 显示 token 用量进度条（已用 / 上限），用户可手动移除不需要的上下文项，精准控制成本
26. **上下文自动管理** -- 当接近 token 上限时，AI 自动摘要旧 Block 而非简单截断，保留关键信息

**记忆系统**
27. **AI 记住你的项目** -- 项目规则（`.aineer/`）、代码风格、技术栈自动识别，跨 Session 持久化
28. **AI 记住你的偏好** -- 常用命令模式、问题解决路径、个人习惯逐步学习
29. **AI 记住历史决策** -- "上次我们选了 Postgres 而不是 MySQL 因为..."，可召回的决策日志

**多渠道触达**
30. **飞书/微信/WhatsApp** -- 同一个 AI 大脑，在手机上也能下指令、查状态、审批工具调用
31. **Gateway HTTP API** -- 其他工具（Cursor、Continue.dev、自建系统）可接入同一个 AI 后端

**持久化与协作**
32. **每个 Tab = 一个持久化开发对话** -- Shell 历史 + AI 对话 = 可保存/恢复的 Session
33. **Session 分享** -- 一键生成 Session 的只读分享链接（通过 Gateway），同事可查看完整 Block 流
34. **Block 引用链接** -- 每个 Block 有唯一 ID，可生成 `aineer://block/<id>` 链接，在团队中引用特定的命令或 AI 回答
...
pub struct ThemeColors {
    ...
    // ... ~200 tokens total (terminal, syntax highlight, git diff, etc.)
}
```
...
```
aineer/                             # 全新画布
├── Cargo.toml                      # workspace root（全新）
├── RELEASE_CHANNEL                 # "dev" | "nightly" | "preview" | "stable"
├── crates/
│   │
│   │  ========= 回收的框架无关 crate（从 ../aineer-legacy/ 复制） =========
│   ├── protocol/                   # 共享类型、事件、凭证、hooks
│   ├── api/                        # Provider HTTP 客户端 + 流式传输
│   ├── engine/                     # Agent 引擎：session、config、prompt、sandbox
│   ├── tools/                      # AI 可调用工具
│   ├── gateway/                    # 嵌入式 OpenAI 兼容 HTTP 代理
│   ├── mcp/                        # MCP 客户端
│   ├── lsp/                        # LSP 客户端
│   ├── plugins/                    # 插件系统
│   ├── cli/                        # TUI REPL（独立前端）
│   │
│   │  ========= 全新 crate =========
│   ├── provider/                   # [新] Provider 注册表（借鉴 Zed language_model）
│   │   └── src/
│   │       ├── lib.rs              # ProviderRegistry, SelectedModel, Provider trait
│   │       ├── credentials.rs      # 凭证管理（env + 系统钥匙串）
│   │       ├── anthropic.rs        # Anthropic provider
│   │       ├── openai.rs           # OpenAI provider
│   │       ├── google.rs           # Google/Gemini provider（新增）
│   │       ├── ollama.rs           # Ollama provider（升级为一等）
│   │       ├── deepseek.rs         # DeepSeek provider（新增）
│   │       ├── mistral.rs          # Mistral provider（新增）
│   │       ├── openrouter.rs       # OpenRouter provider（升级）
│   │       ├── xai.rs              # xAI/Grok provider
│   │       ├── lmstudio.rs         # LM Studio provider（新增）
│   │       └── custom.rs           # OpenAI-compatible 自定义 provider
│   │
│   ├── settings/                   # [新] 统一设置系统（借鉴 Zed settings + settings_content）
│   │   └── src/
│   │       ├── lib.rs              # SettingsStore, 深度合并, 文件监听
│   │       ├── schema.rs           # SettingsContent 类型定义（所有配置项）
│   │       └── loader.rs           # 加载层级: user → project → local
│   │
│   ├── theme/                      # [新] 主题系统（借鉴 Zed theme）
│   │   └── src/
│   │       ├── lib.rs              # ThemeColors, ThemeStyles, ActiveTheme trait
│   │       ├── colors.rs           # ~200 语义 color token
│   │       └── presets.rs          # 内置暗/亮主题
│   │
│   ├── terminal/                   # [新] 终端后端（纯逻辑，无 UI 框架依赖）
│   │   └── src/
│   │       ├── lib.rs              # Terminal struct, TerminalContent 快照
│   │       ├── pty.rs              # PTY 管理（alacritty_terminal）
│   │       └── mappings.rs         # 键盘映射（框架无关类型）
│   │
│   ├── ui/                         # [新] GPUI UI 组件库
│   │   ├── Cargo.toml              # deps: gpui 0.2.2, gpui-component 0.5.1
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── blocks/             # Block 渲染器
│   │       │   ├── mod.rs          # Block enum + BlockView trait
│   │       │   ├── command.rs      # CommandBlock（内嵌 TerminalElement）
│   │       │   ├── ai.rs           # AIBlock（Markdown + 可执行代码块）
│   │       │   ├── tool.rs         # ToolBlock（工具调用卡片）
│   │       │   ├── system.rs       # SystemBlock（信息/错误提示）
│   │       │   └── diff.rs         # DiffBlock（内联 diff 预览）
│   │       ├── stream_view.rs      # [核心] UnifiedStreamView
│   │       ├── input_bar.rs        # SmartInputBar
│   │       ├── terminal_element.rs # GPUI 终端渲染（借鉴 Zed terminal_element）
│   │       ├── explorer.rs         # 文件树面板
│   │       ├── diff_viewer.rs      # Diff 查看器（4 模式）
│   │       ├── settings_panel.rs   # 设置面板（8 页）
│   │       ├── model_picker.rs     # 模型选择器
│   │       ├── command_palette.rs  # 命令面板
│   │       ├── activity_bar.rs     # 左侧活动栏
│   │       ├── status_bar.rs       # 底部状态栏
│   │       └── toast.rs            # 通知 Toast
│   │
│   ├── app/                        # [新] 桌面应用入口
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── main.rs             # CLI vs GUI 分发
│   │       ├── application.rs      # GPUI Application 启动
│   │       ├── workspace.rs        # AineerWorkspace: 三栏布局
│   │       ├── tabs.rs             # TabManager
│   │       ├── agent.rs            # Agent 通信桥接
│   │       └── session.rs          # Session 持久化
│   │
│   ├── memory/                      # [新] 记忆系统（项目知识 + 个人偏好 + 决策日志）
│   ├── channels/                    # [新] 多渠道触达（飞书/微信/WhatsApp bot 适配层）
│   ├── auto_update/                 # [新] 自动更新（借鉴 Zed）
│   └── release_channel/            # [新] Release channel（借鉴 Zed）
│
├── script/                         # 打包脚本（借鉴 Zed）
│   ├── bundle-mac
│   ├── bundle-windows.ps1
...
| 新 Crate | 主要营养来源 | 核心差异 |
|----------|-------------|---------|
| **`provider`** | Zed `language_model/` + `language_models/` + `credentials_provider/` | 去掉 Zed Cloud；纯 Rust struct 非 GPUI Entity；对接 Aineer 的 `api` crate |
| **`settings`** | Zed `settings/` + `settings_content/` | 简化为 3 层（user/project/local）；字段名统一；深度合并不覆盖未知 key |
| **`theme`** | Zed `theme/` | 简化为暗/亮两预设 + 自定义 JSON；去掉 Extension/Registry/FontFamilyCache |
| **`terminal`** | 旧 Aineer `terminal/` + Zed `terminal/` | 去掉所有 egui 依赖；纯 `alacritty_terminal` + 框架无关 `TerminalContent` 快照 |
| **`ui`** | Zed `terminal_view/` + `settings_ui/` + `project_panel/` + `git_ui/` + `command_palette/` | 全部适配 gpui 0.2.2 + gpui-component 0.5.1；核心创新是 UnifiedStreamView |
| **`app`** | 旧 Aineer `app/` (agent.rs, session.rs) | GPUI Application 入口；无 eframe/egui/wgpu |
| **`memory`** | 无直接对应（参考 Cursor Rules / Windsurf Memories 理念） | 项目知识 + 个人偏好 + 决策日志；JSON Lines 存储；注入 system prompt |
| **`channels`** | 无（全新） | 飞书/微信/WhatsApp bot 适配层；统一消息协议，复用同一 Agent 引擎 |
| **`auto_update`** + **`release_channel`** | Zed 同名 crate | 替换 URL/channel/bundle ID |

### 彻底丢弃

- ~~`crates/ui/`~~（egui UI 组件）→ 不存在于新项目
- ~~`crates/terminal/`~~（egui terminal widget）→ 新 `terminal` crate 重写
- ~~`crates/app/`~~（eframe 入口）→ 新 `app` crate 重写

---

## 5. 从 Zed 借鉴的模块清单

所有代码从 `../zed/` 复制后，**适配到 crates.io `gpui 0.2.2` API**。
...
| **Settings UI** | `crates/settings_ui/src/` | `SettingsWindow` 分页 + `SettingField` 控件绑定 | 裁剪为 8 页；适配 gpui 0.2.2 |
| **Theme** | `crates/theme/src/` (colors.rs, styles.rs, accents.rs) | ThemeColors (~200 语义 token) + ThemeStyles | 去掉 Extension/Registry；Hsla 对齐 gpui 0.2.2 |
| **终端渲染** | `crates/terminal_view/src/terminal_element.rs` | `layout_grid` + `paint` + cursor | 适配 gpui 0.2.2 Element trait；Inline + Fullscreen |
| **终端后端** | `crates/terminal/src/terminal.rs` | Terminal struct + TerminalContent 快照 | 参考设计；纯 Rust 无 GPUI 依赖 |
| **键盘映射** | `crates/terminal/src/mappings/keys.rs` | Keystroke → alacritty 字节映射 | 适配 gpui 0.2.2 Keystroke |
...
| **Command Palette** | `crates/command_palette/` | fuzzy match + Action 列表 | 适配 gpui 0.2.2 Action 系统 |
...
GPUI 使用 `smol`/`async-task`，Aineer 后端使用 `tokio`。桥接方式：

```rust
// crates/app/src/bridge.rs

pub struct TokioBridge {
    rt: tokio::runtime::Runtime,    // 2-4 worker threads
}

impl TokioBridge {
    pub fn spawn_bridged<F, T>(
        &self,
        cx: &mut App,              // GPUI context
        entity: &Entity<T>,
        future: F,
    ) where
        ...
        T: 'static,
    {
        ...
        // tokio side: run future, send results through channel
        self.rt.spawn(async move {
            future.await;
            tx.send(()).ok();
        });

        // GPUI side: poll channel, notify entity
...
# Internal crates
aineer-protocol = { path = "crates/protocol" }
aineer-api = { path = "crates/api" }
aineer-provider = { path = "crates/provider" }
aineer-settings = { path = "crates/settings" }
aineer-theme = { path = "crates/theme" }
aineer-terminal = { path = "crates/terminal" }
aineer-engine = { path = "crates/engine" }
aineer-tools = { path = "crates/tools" }
aineer-gateway = { path = "crates/gateway" }
aineer-mcp = { path = "crates/mcp" }
aineer-lsp = { path = "crates/lsp" }
aineer-plugins = { path = "crates/plugins" }
aineer-cli = { path = "crates/cli" }
aineer-ui = { path = "crates/ui" }
```
...
**`crates/terminal/Cargo.toml`**（新，无 egui）
...
| **UI 快照测试** | GPUI 组件渲染 | gpui test utilities | Block 渲染、InputBar 状态切换、Settings 表单 |
| **Design Token 一致性** | token 覆盖率 | 自定义 lint | 扫描 UI 代码，确保 0 硬编码色值/字号/间距 |
| **主题切换测试** | 暗/亮主题视觉 | gpui snapshot | 所有组件在 Dark/Light 主题下渲染对比 |
| **对比度检测** | 可访问性合规 | 自定义检查脚本 | 所有 (text, background) token 对的 WCAG AA 对比度 >= 4.5:1 |
| **E2E 测试** | 完整用户流程 | 手动 + 脚本 | 启动 → 输入命令 → AI 对话 → Agent 执行 → 设置修改 |
...
graph TB
  ...
  subgraph design_sys [篇二 · UI 设计系统]
    DS_Tokens["Design Tokens（~200 语义色彩 + 字体 + 间距）"]
    DS_Components["组件规范（Block/InputBar/Chrome/Panel）"]
    DS_Motion["动效系统（缓动 + 时长 + 触发条件）"]
    DS_Theme["主题架构（ThemeColors → GPUI Hsla 映射）"]
  end

  subgraph tech [篇三 · 技术架构]
    ThemeCrate["crates/theme/ — ThemeColors struct"]
    UICrate["crates/ui/ — 组件实现"]
    AppCrate["crates/app/ — 布局 + 事件桥接"]
  end

  P1 -->|定义交互| DS_Components
  P2 -->|定义信息展示| DS_Components
  P3 -->|定义 Agent 可视化| DS_Components
  DS_Tokens -->|Rust struct 映射| ThemeCrate
  DS_Components -->|GPUI Element 实现| UICrate
  DS_Motion -->|GPUI animation API| UICrate
  DS_Theme -->|ActiveTheme trait| ThemeCrate
  ThemeCrate --> UICrate
  UICrate --> AppCrate
```
...
```mermaid
graph TB
  subgraph desktop [桌面应用 crates/app]
    Main[main.rs] --> App[application.rs]
    App --> WS[workspace.rs]
    WS --> Tabs[tabs.rs]
    WS --> Agent[agent.rs]
    WS --> Session[session.rs]
    WS --> SearchEngine[search/]
  end

  subgraph ui_layer [UI 层 crates/ui]
    StreamView[UnifiedStreamView]
    InputBar[SmartInputBar]
    TermEl[TerminalElement]
    Blocks["Block 渲染器（6 种）"]
    Explorer[ExplorerPanel]
    DiffView[DiffViewer]
    SettingsUI[SettingsPanel]
    ContextPanel[ContextPanel]
    Chrome["Chrome（ActivityBar/StatusBar/Toast/Palette）"]
  end

  subgraph design_infra [设计基础设施]
    ThemeSys["theme/ — ThemeColors + 语义 Token"]
    DesignTokens["Design Tokens → GPUI Hsla 映射"]
  end

  subgraph new_infra [新基础设施]
  ...
  ThemeSys --> DesignTokens
  DesignTokens -.->|cx.theme| StreamView
  DesignTokens -.->|cx.theme| InputBar
  DesignTokens -.->|cx.theme| Chrome
  ...
  TermBackend -.->|alacritty_terminal| TermEl
```
...
| UI 设计系统 Token | Rust 代码位置 | 类型 |
|-------------------|-------------|------|
| `background`, `background.surface`, ... | `ThemeColors::background`, `::background_surface` | `Hsla` |
| `accent.ai`, `accent.agent` | `ThemeColors::accent_ai`, `::accent_agent` | `Hsla` |
| `font.mono`, `font.sans` | `ThemeStyles::font_mono`, `::font_sans` | `SharedString` |
| `font.size.*` | `ThemeStyles::font_size_base`, ... | `Pixels` |
| `space.*` | `ThemeStyles::space_2`, `::space_3`, ... | `Pixels` |
| `radius.*` | `ThemeStyles::radius_sm`, `::radius_md`, ... | `Pixels` |
| `shadow.*` | `ThemeStyles::shadow_sm`, `::shadow_md`, ... | `BoxShadow` |
| `ease.*` | `motion::EASE_DEFAULT`, `::EASE_SPRING`, ... | `cubic_bezier` |

**使用方式：** 组件通过 `cx.theme()` 获取当前主题的 `ThemeColors` 和 `ThemeStyles`，所有视觉属性均通过 token 引用，不在组件代码中硬编码任何色值、字号或间距。
...
### Phase 1: GPUI 基础 + 终端（1.5 周）

**1.1 GPUI 环境**
- 新建 `crates/app/Cargo.toml`（deps: `gpui 0.2.2`，所有回收 crate）
- `main.rs`：CLI mode（`--cli`）→ `cli` crate；默认 → GPUI Application
- 验证 macOS `cargo run` 能打开空窗口

**1.2 主题系统**
- 新建 `crates/theme/`
- 从 Zed `crates/theme/src/styles/colors.rs` 适配 `ThemeColors`（~200 语义 token）
- 适配 gpui 0.2.2 `Hsla` / `Rgba` 类型
- 实现 `ActiveTheme` trait（`cx.theme()` 全局访问）
- 内置 One Dark / One Light 两套预设

**1.3 窗口 + 布局**
- `application.rs`：GPUI Application 启动 + 窗口创建
- `workspace.rs`：AineerWorkspace 三栏布局（ActivityBar | 主区域 | Sidebar）
- `tabs.rs`：TabManager（多 Tab，每个 Tab 对应一个 Session）
- 响应式布局：Sidebar 可折叠

**1.4 终端重写**
- 新建 `crates/terminal/`（纯逻辑，无 UI 框架依赖）
  - `pty.rs`：PTY 管理（从旧 terminal 汲取 + Zed `terminal.rs` 参考）
  - `lib.rs`：`Terminal` struct + `TerminalContent` 快照（框架无关）
  - `mappings.rs`：键盘映射（从 Zed `mappings/keys.rs` 适配，用通用 KeyCode）
- 新建 `crates/ui/src/terminal_element.rs`
  - 从 Zed `terminal_view/src/terminal_element.rs` 适配到 gpui 0.2.2
  - `layout_grid` + `paint` + cursor 渲染
  - 支持 Inline（限高嵌入 Block）和 Fullscreen（全屏接管）两种模式

**验收标准：** 能在窗口中看到三栏布局 + 暗色主题；能创建 PTY 并在 TerminalElement 中看到 Shell 提示符。
...
**2.1 Block 模型**
- `crates/ui/src/blocks/mod.rs`：Block enum + 各 Block struct（含 AgentPlanBlock，见第 8 节）
- 每个 Block 类型实现 `BlockView` trait（`render` 方法返回 GPUI Element）
- BlockMeta 统一元数据（id, timestamp, collapsed, parent_id, tags）

**2.2 UnifiedStreamView**
- `crates/ui/src/stream_view.rs`
- GPUI `uniform_list` 虚拟列表
- Block 渲染分发：`match block { Command => ..., AI => ..., AgentPlan => ..., ... }`
- 自动滚动：新 Block 到达时滚动到底部（用户手动上滚时暂停）
- Alternate screen 检测 → 隐藏 StreamView，显示全屏 TerminalElement
- Block 键盘导航：上下箭头在 Block 间移动，Enter 展开/折叠
- Block 内搜索：Cmd+F 高亮匹配

**2.3 AIBlock 渲染**
- gpui-component `Markdown` 渲染 AI 回复
- 可执行代码块：检测 ` ```bash `  → 渲染 [Run] / [Run All] 按钮 → 点击写入 PTY
- 流式更新：`cx.notify()` 逐 token 刷新
...
**2.6 事件桥接**
- Terminal → UI: `mpsc::Receiver<PtyEvent>` → `cx.spawn()` 轮询 → `cx.notify()`
- Agent → UI: `mpsc::Receiver<AgentEvent>` → 同上（含 AgentPlan 进度事件）
- Gateway → UI: `watch::Receiver<GatewayStatus>` → 同上
- UI → Terminal: 直接调用 `Terminal::write()`
- UI → Agent: `AgentHandle::send_chat()` / `AgentHandle::start_plan()` → `mpsc::Sender`
...
graph TD
  subgraph build [构建与打包]
    Cargo[cargo build --release] --> Mac
    Cargo --> Win
    Cargo --> Linux

    subgraph Mac [macOS]
      BundleMac["cargo-bundle → Aineer.app"] --> Sign[codesign]
      Sign --> Notarize[notarytool]
      Notarize --> DMG["hdiutil → .dmg"]
    end

    subgraph Win [Windows]
      BundleWin[cargo build] --> Inno["Inno Setup 6 → .exe"]
      Inno --> WinSign[Azure Trusted Signing]
    end

    subgraph Linux [Linux]
      BundleLin[cargo build] --> Tar[".tar.gz"]
      BundleLin -.-> FP["Flatpak（可选）"]
    end
  end
...
| GPUI 版本 | crates.io `0.2.2` | 独立开源最高版本 |
| 组件库 | `gpui-component 0.5.1` | 60+ 组件含 Markdown/Table/虚拟列表 |
| UI 设计系统 | 语义 Design Token 三层架构 | 原始色板→语义别名→组件映射，确保主题可切换、0 硬编码 |
| 色彩体系 | ~200 语义 token（借鉴 Zed ThemeColors） | 暗色优先 + 三种强调色（primary/ai/agent），覆盖全组件 |
| 回收 crate | 9 个框架无关 crate 直接回收 | 它们本身就是好代码，与 UI 无耦合 |
| Provider 架构 | 借鉴 Zed `LanguageModelRegistry` | 替换旧的字符串检测；10+ Provider；系统钥匙串 |
| 凭证管理 | Env + 系统钥匙串 | 替换 env-only；GUI 用户可在应用内设置 API Key |
| Settings 持久化 | 深度合并（借鉴 Zed `SettingsStore`） | 修复旧 GUI 覆盖 engine 配置的致命问题 |
| Settings UI | 8 页分页 | 新增 AI/Models（核心）+ MCP Servers 页 |
| Terminal | 新 crate，纯 `alacritty_terminal` | 无 UI 框架依赖；可被 GUI 和 CLI 共用 |
| 文件预览 | DiffViewer（4 模式） | 主打 diff，可切换单版本 |
| 输入模式 | Shell / AI Chat / Agent 三模式统一输入框 | 一个入口解决所有交互，降低认知负荷 |
| Agent 执行 | Plan+Execute + 可视化 AgentPlanBlock | 超越 Chat-only 工具，实现自主多步操作 |
| 全文索引 | `tantivy`（Rust 原生） | 跨 Session 搜索；无外部依赖；性能优异 |
| 上下文管理 | 可视化 Context Panel + 自动摘要 | 透明化 AI 认知，让用户掌控成本和质量 |
| 打包工具 | cargo-bundle + hdiutil + Inno Setup | Zed 同款，成熟可靠 |
| 许可证合规 | Apache 2.0 声明 | Zed 代码 Apache 2.0，适配后保留声明 |
...
| 风险 | 影响 | 应对 |
|------|------|------|
| gpui 0.2.2 与 Zed 代码 API 差异过大 | 适配工作量超预期 | 优先用 gpui-component 替代；fallback 到 gpui-unofficial |
| gpui 0.2.2 已知 Bug | 功能受阻 | 查 Zed issues；必要时 fork 打补丁 |
| TerminalElement 适配困难 | Phase 1 延期 | 先用简化版（纯文本 + ANSI 颜色），后期替换 |
| Linux/Windows 兼容问题 | 跨平台受阻 | Phase 1 即验证三平台构建 |
...
| 1 | GPUI-tokio 异步桥接设计 | 两种运行时互操作，高并发风险 |
| 1 | `TerminalElement` 核心渲染（Zed 适配） | GPUI 底层 API，无参考可循 |
