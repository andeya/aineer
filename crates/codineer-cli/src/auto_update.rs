...
const GITHUB_REPO: &str = "andeya/codineer";
const BINARY_NAME: &str = "codineer";
...
fn state_file_path() -> PathBuf {
    let home = std::env::var_os("CODINEER_CONFIG_HOME")
        .map(PathBuf::from)
        .unwrap_or_else(|| dirs_home().join(".codineer"));
...
pub fn render_update_notification(current: &str, latest: &ReleaseInfo) -> String {
    ...
    let _ = writeln!(
        out,
        "  {d}Run:{r} {c}cargo install codineer-cli{r}  {d}or{r}  {c}brew upgrade codineer{r}",
        d = p.dim,
        r = p.r,
        c = p.cyan_fg,
    );
...
pub fn self_update(release: &ReleaseInfo) -> Result<PathBuf, String> {
    let asset_url = release.asset_download_url.as_deref().ok_or_else(|| {
        format!(
            "No prebuilt binary for this platform ({}/{}). \
             Install manually: cargo install codineer-cli",
            std::env::consts::OS,
            std::env::consts::ARCH,
        )
    })?;
    ...
    eprintln!("  Downloading codineer {}…", release.version);
    ...
    let tmp_path = dir.join(".codineer-update-tmp");
    ...
    let backup_path = dir.join(".codineer-old");
...
pub fn handle_update_command(action: Option<&str>) -> String {
    match action {
        ...
        Some("check") | None => {
            ...
            match result {
                UpdateCheckResult::UpToDate { current } => {
                    format!("✓ Codineer {current} is up to date.")
                }
                UpdateCheckResult::UpdateAvailable { current, latest } => {
        ...
        Some("apply") => {
            ...
            match result {
                ...
                UpdateCheckResult::UpToDate { current } => {
                    format!("✓ Codineer {current} is already up to date.")
                }
                UpdateCheckResult::CheckFailed { reason } => {
                    format!("✗ Update check failed: {reason}")
                }
...
// ---------------------------------------------------------------------------
// CLI subcommand: `codineer update`
// ---------------------------------------------------------------------------

/// Entry point for `codineer update` CLI subcommand.
pub fn run_update_command() {
    ...
    match result {
        UpdateCheckResult::UpToDate { current } => {
            println!("✓ Codineer {current} is up to date.");
        }
        UpdateCheckResult::UpdateAvailable { current, latest }
        | UpdateCheckResult::Dismissed { current, latest } => {
...
mod tests {
    ...
    fn test_release_info(version: &str) -> ReleaseInfo {
        ReleaseInfo {
            ...
            download_url: format!("https://github.com/andeya/codineer/releases/tag/v{version}"),
    ...
    fn state_persistence_roundtrip() {
        let dir = std::env::temp_dir().join("codineer-test-update-state");
        let _ = std::fs::create_dir_all(&dir);
    ...
    fn asset_filename_matches_ci_convention() {
        ...
        assert_eq!(name, format!("codineer-v0.7.0-{target}.{ext}"));
    }
    ...
    fn asset_download_url_matches_ci_convention() {
        ...
        assert_eq!(
            url,
            format!("https://github.com/andeya/codineer/releases/download/v0.7.0/{expected_name}")
        );
    }
    ...
    fn into_release_info_prefers_verified_api_url() {
        let api_url = "https://cdn.example.com/codineer-special.tar.gz";
        let expected_name = asset_filename("1.0.0");
        let release = GithubRelease {
            tag_name: "v1.0.0".into(),
            html_url: "https://github.com/andeya/codineer/releases/tag/v1.0.0".into(),
            body: None,
            published_at: None,
            prerelease: false,
            draft: false,
    ...
    fn into_release_info_falls_back_to_constructed_url() {
        let release = GithubRelease {
            ...
            html_url: "https://github.com/andeya/codineer/releases/tag/v2.0.0".into(),
            body: None,
            published_at: None,
            prerelease: false,
            draft: false,
