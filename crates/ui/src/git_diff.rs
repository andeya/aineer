...
pub fn compute_status(repo_path: &Path) -> Result<GitStatus, String> {
...
fn compute_changes(repo: &Repository) -> Result<ChangesResult, String> {
...
/// Revert a single hunk by reconstructing the file content with the hunk's
/// changes undone. Uses the stored DiffHunk line data.
pub fn revert_hunk(repo_path: &Path, file_path: &str, hunk: &DiffHunk) -> Result<(), String> {
    let repo = Repository::discover(repo_path).map_err(|e| e.message().to_string())?;
    ...
    let first_new_lineno = hunk
        ...
        .ok_or("Cannot determine hunk position")?;

    // 0-indexed start of this hunk's region in the current (new) file
    let hunk_start = (first_new_lineno as usize).saturating_sub(1);

    // Count how many "new" lines this hunk spans (Add + Context)
    let hunk_new_len = hunk
        .lines
        .iter()
        .filter(|l| l.kind == DiffLineKind::Add || l.kind == DiffLineKind::Context)
        .count();

    // Build the original lines for this hunk region (Delete + Context)
    let original_lines: Vec<&str> = hunk
        .lines
        .iter()
        .filter(|l| l.kind == DiffLineKind::Delete || l.kind == DiffLineKind::Context)
        .map(|l| l.content.trim_end_matches('\n'))
        .collect();

    let mut result: Vec<&str> = Vec::with_capacity(lines.len());
    result.extend_from_slice(&lines[..hunk_start]);
    result.extend_from_slice(&original_lines);
    let after_hunk = hunk_start + hunk_new_len;
    if after_hunk < lines.len() {
        result.extend_from_slice(&lines[after_hunk..]);
    }
    ...
    std::fs::write(&full_path, output).map_err(|e| format!("Failed to write file: {e}"))
}
