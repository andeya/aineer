...
fn config_file_paths() -> ConfigFilePaths {
    if cfg!(target_os = "windows") {
        ConfigFilePaths {
            local_override: r".aineer\settings.local.json",
            project_settings: r".aineer\settings.json",
            global_settings: r"%USERPROFILE%\.aineer\settings.json",
        }
    } else {
        ConfigFilePaths {
            local_override: ".aineer/settings.local.json",
            project_settings: ".aineer/settings.json",
            global_settings: "~/.aineer/settings.json",
        }
    }
}
...
pub(crate) fn print_help_to(out: &mut impl io::Write) -> io::Result<()> {
    ...
    print_help_section(
        ...
        "Quick start",
        &[
            "  aineer                                  Start the interactive REPL",
            "  aineer \"summarize this repo\"            Run one prompt and exit",
            "  aineer prompt \"explain src/main.rs\"     Explicit one-shot prompt",
            "  aineer --resume SESSION.json /status    Inspect a saved session",
        ],
    )?;
    {
    ...
    print_help_section(
        ...
        "Commands",
        &[
            "  aineer help                             Show this help message",
            "  aineer agents                           List configured agents",
            "  aineer skills                           List installed skills",
            "  aineer system-prompt [--cwd PATH] [--date YYYY-MM-DD]",
            "  aineer login [<provider>] [--source <id>] Start the login flow",
            "  aineer logout [<provider>] [--source <id>] Clear saved credentials",
            "  aineer status [<provider>]              Show authentication status",
            "  aineer models [<provider>]              List available models",
            "  aineer config set <key> <value>         Set a configuration value",
            "  aineer config get [<key>]               Show a configuration value",
            "  aineer config list                      List all configuration",
    ...
    print_help_section(
        ...
        &[
            ...
            "  AINEER_WORKSPACE_ROOT               Override workspace root directory",
            "  AINEER_CONFIG_HOME                  Override config directory (default: ~/.aineer)",
    ...
    print_help_section(
        ...
        &[
            "  Anthropic (Claude):  env vars → Aineer OAuth → Claude Code auto-discover",
            ...
            "  Aineer auto-detects saved Claude Code credentials (~/.claude/.credentials.json).",
    ...
    print_help_section(
        ...
        &[
            ...
            &format!("  {:<38}Project context and instructions", ".aineer/AINEER.md"),
            "",
            "  Override the global config dir via AINEER_CONFIG_HOME.",
            "",
            ...
            "  Tip: use `aineer config set <key> <value>` instead of editing JSON manually.",
        ],
    )?;
    print_help_section(
        out,
        ...
        &[
            ...
            "  Usage:     aineer --model ollama/qwen3-coder",
            "             aineer --model groq/llama-3.3-70b-versatile",
            "  Auto:      aineer --model ollama   (picks best coding model)",
            "  Zero-config Ollama: if no API keys are found and Ollama is running,",
            "             Aineer auto-detects it and picks a coding model.",
            "",
    ...
    print_help_section(
        ...
        "Examples",
        &[
            "  aineer --model sonnet \"summarize this repo\"",
            "  aineer --output-format json prompt \"explain src/main.rs\"",
            "  aineer --allowedTools read,glob \"summarize Cargo.toml\"",
            "  aineer --resume session.json /status /diff /export notes.txt",
            "  aineer agents",
            "  aineer /skills",
            "  aineer login",
            "  aineer login anthropic --source claude-code",
            "  aineer status",
            "  aineer models anthropic",
            "  aineer config set model sonnet",
            "  aineer config set fallbackModels '[\"ollama/qwen3-coder\"]'",
            "  aineer init",
        ],
    )
}
...
pub(crate) fn render_repl_help() -> String {
    let mut header: Vec<&str> = vec![
        "Interactive REPL",
        "  Quick start          Welcome panel shows context and `aineer --resume …`; prompt is `❯`. Ask in plain English or use commands below.",
