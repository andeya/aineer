...
fn left_column(p: &Palette, ctx: &BannerContext<'_>, layout: &BannerLayout) -> Vec<String> {
    ...
    let mut lines = vec![
        center_in(&format!("{}Welcome back · Aineer{}", p.bold_white, p.r), lc),
        String::new(),
    ];
    if lc >= LOGO_WIDTH {
        for l in &logo {
...
fn right_column(p: &Palette, ctx: &BannerContext<'_>, layout: &BannerLayout) -> Vec<String> {
    ...
    let resume_cmd = format!("aineer --resume {}", resume.display());
