...
/// Content for `.aineer/.gitignore`.
/// Lines starting with `!` are negated (i.e. explicitly tracked).
const AINEER_GITIGNORE: &str = "\
...
pub(crate) fn initialize_repo(cwd: &Path) -> CliResult<InitReport> {
    let mut artifacts = ensure_aineer_scaffold(cwd)?;

    let content = render_init_aineer_md(cwd);
    artifacts.push(InitArtifact {
        name: "AINEER.md".into(),
        depth: 1,
        status: write_file_if_missing(&cwd.join(".aineer").join("AINEER.md"), &content)?,
    });
...
const AINEER_SUBDIRS: &[&str] = &["plugins", "skills", "agents", "sessions"];
...
fn ensure_aineer_scaffold(root: &Path) -> Result<Vec<InitArtifact>, std::io::Error> {
    let cd = root.join(".aineer");
    ...
    artifacts.push(InitArtifact {
        name: ".gitignore".into(),
        depth: 1,
        status: write_file_if_missing(&cd.join(".gitignore"), AINEER_GITIGNORE)?,
    });
...
pub(crate) fn render_init_aineer_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# AINEER.md".to_string(),
        String::new(),
        "This file provides guidance to Aineer (aineer.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.aineer/settings.json`; reserve `.aineer/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `.aineer/AINEER.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_aineer_md};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!("aineer-init-{nanos}-{id}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".aineer/       created"));
        ...
        assert!(rendered.contains("  AINEER.md      created"));
        let cd = root.join(".aineer");
        assert!(cd.is_dir());
        ...
        assert!(cd.join("AINEER.md").is_file());
        assert_eq!(
        ...
        assert!(gitignore.contains("!AINEER.md"));
        ...
        let aineer_md = fs::read_to_string(cd.join("AINEER.md")).expect("read aineer md");
        assert!(aineer_md.contains("Languages: Rust."));
        assert!(aineer_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        let root = temp_dir();
        let cd = root.join(".aineer");
        fs::create_dir_all(&cd).expect("create .aineer dir");
        fs::write(cd.join("AINEER.md"), "custom guidance\n").expect("write existing aineer md");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first
            .render()
            .contains("AINEER.md      skipped (already exists)"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".aineer/       skipped (already exists)"));
        ...
        assert!(second_rendered.contains("  AINEER.md      skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(cd.join("AINEER.md")).expect("read existing aineer md"),
            "custom guidance\n"
        );
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_aineer_md(Path::new(&root));
