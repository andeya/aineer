...
use egui::Widget;
use egui::{Id, Key, PointerButton, Pos2, Response, Vec2};
...
use crate::render::{self, TerminalGridCache};
...
impl<'a> TerminalView<'a> {
    ...
    fn show(self, state: &mut TerminalViewState, layout: &Response, painter: &egui::Painter) {
        let content = self.backend.sync();
        let allow_cache = content.selectable_range.is_none()
            && !state.is_dragged
            && content.hovered_hyperlink.is_none();
